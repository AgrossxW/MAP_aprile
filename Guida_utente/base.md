...
Il sistema implementa un’architettura **Client–Server** per l’esecuzione dell’algoritmo di clustering QT (*Quality Threshold*).
...
Il **client** permette all’utente di:
...
Per eseguire il sistema è necessario disporre di:

- Java JDK 17 o superiore
- MySQL Server installato e attivo, per la modalità database
- driver MySQL Connector presente nella cartella `Server/libs/`

## Requisiti di rete

- Il **client** e il **server** devono poter comunicare sulla rete.
- Se eseguiti sulla stessa macchina, è sufficiente utilizzare `localhost` (`127.0.0.1`).
- La porta **8080** deve essere libera sul computer in cui viene avviato il server.

> **Attenzione:** se la porta 8080 è già occupata, il server non potrà avviarsi correttamente.
...
Prima di utilizzare la modalità di caricamento da database, è necessario configurare MySQL.

1. Avviare MySQL Server.
2. Aprire un client MySQL, ad esempio MySQL Workbench oppure il terminale.
...
**Attenzione:** Senza l’esecuzione di `mapDB.sql`, la modalità database non funzionerà correttamente.
...
> **Attenzione:** il server deve essere avviato prima del client.
...
Questa modalità consente di caricare e visualizzare cluster **già salvati in precedenza**.
...
Questa modalità consente di caricare i dati da una tabella MySQL ed eseguire il clustering QT.
...
2. Inserire:
   - **Table Name**: nome della tabella
   - **Radius**: valore del raggio da utilizzare per il clustering
...
Dopo ogni esecuzione del clustering in modalità database, il client richiede al server il salvataggio dei risultati in un file **`.dat`**.  
...
- **Examples**  
  Elenca tutti gli elementi appartenenti al cluster.  
...
Si consiglia di evitare la chiusura forzata della finestra del terminale.

---

# 8. Risoluzione dei problemi

Questa sezione descrive i problemi più comuni che l’utente può incontrare durante l’esecuzione e le relative soluzioni.

## 8.1 Errori di connessione e avvio

### Errore: `java.net.ConnectException: Connection refused`

**Causa:** il client non riesce a connettersi al server, ad esempio perché il server non è avviato oppure la porta è errata o bloccata.

**Soluzione:**
- avviare prima il server e solo dopo il client
- verificare che la porta **8080** sia libera sul computer dove gira il server
- se client e server sono su macchine diverse, controllare IP, host e connettività di rete
...
- controllare credenziali e parametri di connessione (host, porta, user, password)
...
- controllare che il nome della tabella inserito sia corretto

### Errore: `EmptySetException` / `NoValueException`

**Causa:** la tabella esiste ma non contiene dati compatibili con l’elaborazione, ad esempio è vuota o presenta valori mancanti.
...
### Comportamento: il menu viene riproposto

**Causa:** è stato inserito un valore non valido nel menu, ad esempio diverso da `1` o `2`.
...
**Causa:** è stato inserito un valore non valido, non numerico oppure `<= 0`.
...
**Significato:** il raggio scelto produce un risultato non utile, tipicamente **un solo cluster** oppure un clustering poco significativo.
...
- riprovare poi con **Load clusters from file** (opzione 1) usando gli stessi valori di tabella e raggio
